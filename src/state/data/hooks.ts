...
export const useAllCurrencies = () => useSelector(state => state.data.currency.ids.map(id => state.data.currency.entities[id]!), shallowEqual);
...
export const useAllNotifications = () =>
    useSelector(state => state.data.notification.ids.map(id => state.data.notification.entities[id]!), shallowEqual);
...
export const useAllTransactions = (filter?: (transaction: Transaction) => boolean) =>
    useSelector(state => {
        const results = state.data.transaction.ids.map(id => state.data.transaction.entities[id]!);
        return filter ? results.filter(filter) : results;
    }, shallowEqual);
