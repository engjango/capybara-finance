...
const updateDropboxState = (dropbox: User["dropbox"]) =>
    CapybaraDispatch(DataSlice.actions.updateUserPartial({ dropbox }));
let token: {
    access_token: string;
    expires: DateTime;
} | null = null;
...
export const maybeSaveDataToDropbox = async () => {
    ...
    const config = CapybaraStore.getState().data.user.entities[StubUserID]!.dropbox;
    ...
    // Generate Zip file
    const zip = new JSZip();
    zip.file("data.json", JSON.stringify(CapybaraStore.getState().data));
