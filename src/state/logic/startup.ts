...
import { CapybaraDexie } from "./database";
...
export const initialiseDemoData = async () => {
    ...
    CapybaraDispatch(DataSlice.actions.setUpDemo(DemoData));
...
export const initialiseAndGetDBConnection = async () => {
    ...
    window.onpopstate = () => CapybaraDispatch(AppSlice.actions.setPageStateFromPath());

    // Set up IDB, if present
    let db = new CapybaraDexie();
    let loadedStateFromIDB = false;
    await db.user
        .get(StubUserID)
        .then(async (user) => {
            if (user) {
                // IDB contains existing Capybara state
...
const initialiseIDBListener = (db: CapybaraDexie, uuid: string) => {
...
const initialiseIDBSyncFromRedux = (db: CapybaraDexie, uuid: string) => {
...
const hydrateReduxFromIDB = async (db: CapybaraDexie) => {
    ...
    CapybaraDispatch(DataSlice.actions.setFromLists(zipObject(DataKeys, values) as unknown as ListDataState));
};
...
export const handleMigrationsAndUpdates = (oldGeneration: number | undefined) => {
    ...
    if (
        generation === 0 || // Refresh caches to deal with https://github.com/Athenodoros/Capybara/issues/8
        generation === 1 || // Refresh caches to deal with https://github.com/Athenodoros/Capybara/issues/13
        ...
        CapybaraDispatch(DataSlice.actions.refreshCaches());
        generation = 4;
    }
    ...
    if (oldGeneration !== generation) {
        ...
        CapybaraDispatch(DataSlice.actions.setUserGeneration(generation));
    }
};

const getDebugVariablesAsync = (db: CapybaraDexie) => async () => {
    if (!debug)
        console.warn(
    ...
    return {
        ...
        updateSyncedCurrencies,
        removeUnusedStatements: () => CapybaraDispatch(DataSlice.actions.removeUnusedStatements()),
        fitAccountUpdateDates: () => CapybaraDispatch(DataSlice.actions.fitAccountLastUpdateDates()),
        restart: () => CapybaraDispatch(DataSlice.actions.restartTutorial()),
        refreshCaches: () => CapybaraDispatch(DataSlice.actions.refreshCaches()),
    };
};
