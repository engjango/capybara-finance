...
export const AccountNotificationDefinition: NotificationRuleDefinition = {
    ...
    display: alert => {
        ...
        return {
            ...
            title: translations[language].accountNotUpdated,
            dismiss: dismiss(contents.id),
            colour: Intents.warning.main,
            buttons: [
                { text: translations[language].uploadStatement, onClick: openStatementDialog(contents.id) },
                { text: translations[language].markCurrent, onClick: markAccountCurrent(contents.id) },
            ],
    ...
    if (!closedProgrammatically)
        CapybaraDispatch(
            DataSlice.actions.updateUserPartial({
                accountOutOfDate: CapybaraStore.getState().data.user.entities[StubUserID]!.accountOutOfDate.concat([
                    account,
                ]),
            })
        );
    ...
    CapybaraDispatch(
        AppSlice.actions.setDialogPartial({
            ...
            import: {
                ...
                account: CapybaraStore.getState().data.account.entities[id]!,
            },
        })
    );
};
const markAccountCurrent = (id: ID) => (close: () => void) => {
    CapybaraDispatch(DataSlice.actions.updateAccount({ id, changes: { lastUpdate: getTodayString() } }));
    close();
};
...
const OldAccountContents: React.FC<{ id: ID; age: number }> = ({ id, age }) => {
    ...
    return (
        <NotificationContents>
            {translations[language].theAccount}
            <Box
                sx={{
            ...
            {translations[language].hasNotBeenUpdatedIn} <OrangeNotificationText>{age}</OrangeNotificationText>{' '}
