...
const setStatementState = (state: DialogFileState) =>
    CapybaraDispatch(AppSlice.actions.setDialogPartial({ id: "import", import: state }));
const getDialogState = () => CapybaraStore.getState().app.dialog;
const getDataState = () => CapybaraStore.getState().data;
...
export const changeStatementDialogAccount = (id?: ID) => {
    ...
    const account = CapybaraStore.getState().data.account.entities[id!];
...
export const importStatementsAndClearDialog = (shouldRunRules: boolean, shouldDetectTransfers: boolean) =>
    batch(() => {
        ...
        const page = CapybaraStore.getState().app.page;
        CapybaraDispatch(
