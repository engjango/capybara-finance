...
export const EditableTextValue: React.FC<EditableTextValueProps> = ({
   ...
   return (
      <TextField
         ...
         variant="filled"
         placeholder={
            value === undefined ? `${translations[language].mixed}` : placeholder || `${translations[language].none}`
         }
      ...
      <ObjectSelector
         ...
         placeholder={
            allowUndefined ? (
               <>
                  <Box sx={iconSx} />
                  <TransactionTableMixedTypography>{ translations[language].mixed }</TransactionTableMixedTypography>
               </>
            ) : undefined
         }
      >
         {button || (
            <Button sx={ObjectDropdownButtonSx} variant="contained" component="div" color="inherit" disabled={disabled}>
               ...
               <ObjectDropdownLabelTypography
                  ...
                  style={{color: "#080707"}}
               >
                  {option?.name || `${ translations[language].mixed }`}
               </ObjectDropdownLabelTypography>
            </Button>
         )}
      </ObjectSelector>
