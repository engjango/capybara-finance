   ...
   ({ transaction: tx, updateState, fixed }) => {
      ...
      const MissingText = (
         <TransactionTableMixedTypography variant="body1" noWrap={true}>
            {translations[language].mixed}
         </TransactionTableMixedTypography>
      );

      const getCurrencyDisplay = (value: number | null | undefined, missing?: boolean, isBalance: boolean = false) =>
         value !== undefined && value !== null ? (
            <TransactionTableCompoundContainer>
               {currency && (
                  <SubtextTypography variant="caption" sx={missing ? TransactionTableSxProps.MissingValue : undefined}>
                     {currency.symbol}
                  </SubtextTypography>
               )}
               <Typography
                  ...
                  style={{ fontWeight: 100, color: isBalance ? '#67656a' : value >= 0 ? 'green' : 'red' }}
               >
      ...
      return (
         <>
            ...
            {fixed?.type !== 'category' || fixed.nested === true ? (
               <TransactionTableCategoryContainer>
                  {category && category.id !== PLACEHOLDER_CATEGORY_ID ? (
                     <TransactionTableCompoundContainer
                        sx={
                           category.id === TRANSFER_CATEGORY_ID || category.id === FAKE_BALANCE_READING_CATEGORY_ID
                              ? BuiltinCategorySx
                              : undefined
                        }
                     >
                        {getCategoryIcon(category, CategoryIconSx)}
                        <Typography noWrap={true} style={{fontWeight: 100}}>
            ...
            <TransactionTableBalanceContainer>
               {getCurrencyDisplay(tx.recordedBalance ?? tx.balance, tx.recordedBalance === null, true)}
            </TransactionTableBalanceContainer>
            <TransactionTableStatementContainer>
               {statement ? (
                  ...
                  <Tooltip title={translations[language].mixed}>
            ...
            <TransactionTableActionsContainer>
               <IconButton size="small" onClick={handleStartEdit} style={{ color: "#fff", padding: "8px" }}>
...
const StatementDescriptionIcon = styled(Description)({
   opacity: 0.3,
});

const StatementHelpIcon = styled(Help)({
   opacity: 0.3,
});
