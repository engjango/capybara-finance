...
export const CategoriesBarChart: React.FC = () => {
    ...
    return (
        <Section
            title={translations[language].budgetTrend}
            headers={
                <Select
                    value={sign}
                ...
                >
                    <MenuItem value="all">{translations[language].allCategories}</MenuItem>
                    <MenuItem value="credits">{translations[language].incomeCategories}</MenuItem>
                    <MenuItem value="debits">{translations[language].expenseCategories}</MenuItem>
                </Select>
            }
        >
...
const CHART_STYLES = { height: '100%', display: 'flex', justifyContent: 'center' } as const;
...
const setChartSign = handleSelectChange((summarySign: CategoriesPageState['summarySign']) =>
    CapybaraDispatch(AppSlice.actions.setCategoriesPagePartial({ summarySign }))
);
