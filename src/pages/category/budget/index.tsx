...
export const CategoryPageBudgetSummary: React.FC = () => {
    ...
    const openEditView = useCallback(
        () => CapybaraDispatch(AppSlice.actions.setDialogPartial({ id: 'category', category })),
        [category]
    );
    ...
    if (!toplevel.budgets)
        return (
            <Section title={translations[language].budget} PaperSx={SectionSx}>
                <NonIdealState
                    icon={AssignmentLate}
                    title={translations[language].noBudgetAdded}
                    subtitle={translations[language].noBudgetAddedDesc}
                    action={
                        <Button onClick={openEditView} startIcon={<OpenInNew />}>
                            {translations[language].addBudget}
                        </Button>
                    }
                />
            </Section>
    ...
    return (
        <Section title={translations[language].budget} PaperSx={SectionSx}>
            <FillbarSummaryNumbers
                ...
                budget={budget}
                date={getToday()}
                subtitle={translations[language].ofBudgetSoFar}
            ...
            <FillbarSummaryNumbers
                ...
                subtitle={translations[language].ofBudgetLastMonth}
                functions={chartFunctions}
            />
