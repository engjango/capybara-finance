...
export const DialogCurrenciesView: React.FC = () => {
    ...
    const render = (currency: Currency) => (
        <CurrencyBox>
            ...
            {currency.sync && (
                <Tooltip title={translations[language].automaticExchangeRates}>
        ...
        <DialogMain onClick={remove}>
            ...
            <DialogContents>
                {working ? (
                    ...
                    <NonIdealState
                        icon={Euro}
                        title={translations[language].currencies}
                        subtitle={translations[language].currenciesDesc}
...
const updateDefaultCurrency = (id: ID) => CapybaraDispatch(DataSlice.actions.setDefaultCurrency(id));
const DefaultCurrencySelector: React.FC = () => {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 50 }}>
            <Typography variant="subtitle2">{translations[language].defaultCurrency}</Typography>
            <ObjectSelector
                options={currencies}
...
const createNewCurrency = (): Currency => ({
    id: getNextID(CapybaraStore.getState().data.currency.ids),
    ticker: 'NCD',
...
const EditCurrencyView: React.FC = () => {
    ...
    const TickerDescriptions = {
        currency: translations[language].symbol,
        crypto: translations[language].symbol,
        stock: translations[language].ticker,
    };
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <CustomTextField
                    ...
                    label={
                        isDemoError ? (
                            <>
                                {translations[language].noCurrencyTokenSee}{' '}
    ...
    return (
        ...
        >
            <EditValueContainer label={translations[language].display}>
                <CustomTextField
                ...
                <CustomTextField
                    ...
                    label={translations[language].ticker}
                />
                <ColourContainerBox>
            ...
            <EditTitleContainer title={translations[language].exchangeRates} />
            <EditValueContainer label={translations[language].source}>
                <ToggleButtonGroup
                    size="small"
                ...
                >
                    ...
                    >
                        <Tooltip title={translations[language].manualRates}>
                    ...
                    >
                        <Tooltip title={translations[language].pinToCurrency}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <EuroSymbol fontSize="small" />
                                <Typography variant="caption">{translations[language].currency}</Typography>
                            </Box>
                        </Tooltip>
                    </ToggleButton>
                    <ToggleButton
                    ...
                    >
                        <Tooltip title={translations[language].pinToCrypto}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <Money fontSize="small" />
                                <Typography variant="caption">{translations[language].crypto}</Typography>
                            </Box>
                        </Tooltip>
                    </ToggleButton>
                    <ToggleButton
                    ...
                    >
                        <Tooltip title={translations[language].pinToStock}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <ShowChart fontSize="small" />
                                <Typography variant="caption">{translations[language].stock}</Typography>
                            </Box>
                        </Tooltip>
                    </ToggleButton>
                </ToggleButtonGroup>
...
const useCurrencyRatesInput = (working: Currency, inputs?: React.ReactNode) => {
    const getOriginalRates = useCallback(() => {
        const actual = CapybaraStore.getState().data.currency.entities[working.id];
...
const goToTokenSetup = () =>
    CapybaraDispatch(AppSlice.actions.setDialogPartial({ currency: undefined, settings: 'currency', id: 'settings' }));
